...
/**
 * Lifecycle callbacks for the `restuarant` model.
 */
...
module.exports = {
  ...
  beforeSave: async (model, attrs, options) => {
    if (options.method === 'insert' && attrs.name) {
      model.set('slug', slugify(attrs.name, '-', slugify.transformers.lowercase));
    } else if (options.method === 'update' && attrs.name) {
      attrs.slug = slugify(attrs.name, '-', slugify.transformers.lowercase);
    }
  },
